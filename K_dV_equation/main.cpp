...
{
  Eigen::VectorXcd G_hat(N), u(N), sum(N), u_hat(N);
  sum.setZero();
  for (int i = 0; i < N; i++)
  {
    double k = i - N / 2;
    u_hat(i) = U_hat(i) * exp(i_unit * alpha * k * k * k * t);
  }
  eigen_to_fftw(u_hat, in);
  fftw_execute(p_backward);
...
int main()
{
  ...
  for (int i = 0; i < steps; i++)
  {
    ...
    if (i % 10 == 0)
    {
      if (is_linear_deleted)
      {
        ...
        {
          double k = l - N / 2;
          u_hat(l) = U_hat(l) * exp(i_unit * alpha * k * k * k * (t + dt));
        }
      }
      eigen_to_fftw(u_hat, in);
      fftw_execute(p_backward);
  ...
  if (is_linear_deleted)
  {
    ...
    {
      double k = l - N / 2;
      u_hat(l) = U_hat(l) * exp(i_unit * alpha * k * k * k * static_cast<double>(steps) * dt);
    }
  }
